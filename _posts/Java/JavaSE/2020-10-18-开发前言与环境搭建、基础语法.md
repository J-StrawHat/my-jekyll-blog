...
+ 以数字0开头，由0~7组成的数是八进制。故下面这条语句是**错误**的。
...
+ 同样地，若构造方法中没有显式地给对象的数据域赋值，那么系统会自动地为它们赋为默认值。
...
+ 类属性是在类加载时（第一次出现该类，故只会调用一次）初始化

+ 类加载->static->实例初始化块->构造方法

+ 数据封装的优点围绕于“隐藏”、“暴露”。

+ 对象数组实际上是引用变量的数组

  <img src="https://gitee.com/j__strawhat/MyImages/raw/master/20210422141212.png"/>

+ 一个类是不变类，必须满足如下条件：

  + 所有的数据域都是私有的；
  + 没有修改器
  + 没有一个**返回指向可变数据域的引用**的访问器方法

+ `this`可以在**构造方法内部**用于调用同一个类其他的构造方法，且放在方法内的**第一行**（否则出现编译错误）
...
+ 子类的构造方法若不显式调用父类的构造，则会调用父类的无参构造方法

  ![image-20210510091444251](C:\Users\luffy\AppData\Roaming\Typora\typora-user-images\image-20210510091444251.png)
